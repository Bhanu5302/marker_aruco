...
rot = []
ids = 0
marker_size = 0.091  # 0.166#0.091 #in meters
...
class image_converter:
    ...
    def __init__(self):
        ...
        self.between_pub = rospy.Publisher("aruco_points", PoseStamped, queue_size=10)
        self.bridge = CvBridge()
        self.calibrate_camera = rospy.Subscriber("/zed/zed_nodelet/left/camera_info", CameraInfo,
                                                 self.callbackCalibrate)
        self.image_sub = rospy.Subscriber("/zed/zed_nodelet/left_raw/image_raw_gray", Image, self.callback)

    def callback(self, data):

        global distortion_coefficients
        global matrix_coefficients, translation, rotation, rot, ids
        ...
        if len(corners) > 0:
            for i in range(0, len(ids)):
                ...
                rvec, tvec, _objPoints = cv2.aruco.estimatePoseSingleMarkers(corners[i], marker_size,
                                                                             matrix_coefficients,
                                                                             distortion_coefficients)
                ...
                mode = "start"
                if mode == "start":
                    self.publishPostPose(ids[i], tvec[0][0][0], tvec[0][0][1], tvec[0][0][2], quaternion[0],
                                         quaternion[1],
        ...
        translation = (positionZ, positionX, positionY)
    ...
    def publishPostPose(self, id, pX, pY, pZ, oX, oY, oZ, oW):
        global postPose, ids
        postPose = PoseStamped()
        postPose.header.stamp = rospy.Time.now() - rospy.Duration(0.1)
        postPose.header.frame_id = str(id[0])
        ...
        translation_t = (pX, pY, pZ)
        rotation_t = [oX, oY, oZ, oW]
        rotation_t = np.array(rotation_t)
        # -------------transform broadcaster----------
        b = TransformBroadcaster()
        rot_eul = euler_from_quaternion(rotation_t)
        # print((rot_eul))
        quat = quaternion_from_euler(2.7, 0.0, 0.2)

        # print(type(target))
        if id[0] == 1:
            b.sendTransform(translation_t,
                            tf.transformations.quaternion_from_euler(rot_eul[0] - 1.57, rot_eul[1] - 1.57, rot_eul[2]),
                            rospy.Time.now(), 'marker_1', 'zed_left_camera_optical_frame')  # str(id[0])
        if id[0] == 3:
            b.sendTransform(translation_t,
                            tf.transformations.quaternion_from_euler(rot_eul[0] - 1.57, rot_eul[1] - 1.57, rot_eul[2]),
                            rospy.Time.now(), 'marker_3', 'zed_left_camera_optical_frame')  # str(id[0])
        # --------------------------------------------------------------
        self.between_pub.publish(postPose)
